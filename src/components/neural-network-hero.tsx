...
const fragmentShader = `
  ...
  void main() {
    ...
    // ZielHub logo colors
    vec3 color1 = vec3(0.118, 0.518, 0.761); // #1e84c2 - blue medium
    vec3 color2 = vec3(0.310, 0.753, 0.859); // #4fc0db - cyan light
    vec3 color3 = vec3(0.039, 0.055, 0.118); // darker version of #191e3a
    vec3 color4 = vec3(0.0, 0.220, 0.392);   // darker version of #004a84

    // Mix colors based on pattern values
    vec3 mixColor1 = mix(color3, color1, pattern.x);
    vec3 mixColor2 = mix(color4, color2, pattern.y);
    vec3 finalColor = mix(mixColor1, mixColor2, pattern.z * 0.5 + 0.5);
